...
def add_predict(df,EPM,features_list):
    ...
    df_ = df.copy()
    df_.loc[:, 'classification'] = classes_pred
...
def flow_base(file,pars, EPM, EPM_dense_output_model, ADM, blank=None,ms2=None):
    ...
    # Seperate the results based on the classification
    df_Se = df_f_result[df_f_result['classification']==3]
    df_B = df_f_result[df_f_result['classification']==4]
    df_Fe = df_f_result[df_f_result['classification']==5]
