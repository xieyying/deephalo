...
class Dereplication():
    ...
    def dereplication(self):
        ...
        for idx,row in self.Deephalo_output.iterrows():
            ...
            dereplications = {'compound_names':[],'intensity_score':[],'error_ppm':[],'Smiles':[]}
            ...
            if self.data_.empty:
                ...
                for idx_,row_ in self.data_.iterrows():
                    ...
                    try:
                        dereplications['Smiles'].append(row_['Smiles'])
                    except:
                        dereplications['Smiles'].append('None')
                ...
                self.Deephalo_output.loc[idx,'Smiles'] = dereplications.loc[dereplications['intensity_score'].idxmax(),'Smiles']
...
if __name__ == '__main__':
    ...
    files = [file for file in files if file.endswith('dereplication.csv')]
    dereplication_folder_ = Deephalo_output_result.replace('halo','dereplication_with_known')
    os.makedirs(dereplication_folder_,exist_ok=True)
    for file in files:
        df = pd.read_csv(os.path.join(dereplication_folder,file))
        df = df[df['error_ppm']!=1e6]
        if df.shape[0] > 0:
            df.to_csv(os.path.join(dereplication_folder_,file),index=False)
