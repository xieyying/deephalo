    ...
    def dereplication(self):
        ...
        for dataname, data in self.data.items():
            ...
            for idx, row in self.Deephalo_output.iterrows():
                ...
                data_h = data[(abs(data['M+H'] - (row['mz']*row['charge']-((row['charge']-1)*1.007825))) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_na = data[(abs(data['M+Na'] - (row['mz']*row['charge']-((row['charge']-1)*1.007825)) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_nh4 = data[(abs(data['M+NH4'] - (row['mz']*row['charge']-((row['charge']-1)*1.007825)) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                ...
                if data_.empty:
                    ...
                    for idx_, row_ in data_.iterrows():
                        ...
                        error_h = abs(row['mz'] - row_['M+H']) / row['mz'] * 1e6
                        error_na = abs(row['mz'] - row_['M+Na']) / row['mz'] * 1e6
                        error_nh4 = abs(row['mz'] - row_['M+NH4']) / row['mz'] * 1e6
