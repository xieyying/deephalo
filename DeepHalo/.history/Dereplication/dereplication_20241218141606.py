...
class Dereplication:
    ...
    def dereplication(self):
        ...
        for dataname, data in self.data.items():
            ...
            data['compound_m_plus_h'] = pd.to_numeric(data['compound_m_plus_h'], errors='coerce')
            data['compound_m_plus_na'] = pd.to_numeric(data['compound_m_plus_na'], errors='coerce')
            data['compound_m_plus_nh4'] = pd.to_numeric(data['compound_m_plus_nh4'], errors='coerce')
            ...
            for idx, row in self.Deephalo_output.iterrows():
                ...
                data_h = data[(abs(data['compound_m_plus_h'] - row['mz']) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_na = data[(abs(data['compound_m_plus_na'] - row['mz']) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_nh4 = data[(abs(data['compound_m_plus_nh4'] - row['mz']) <= row['mz'] * 1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_ = pd.concat([data_h, data_na, data_nh4], ignore_index=True)
...
if __name__ == '__main__':
    databases = {'NPAtlas':r'K:\open-database\NPAtlas\V2024_03\dereplicate_database_base_bacteria.csv',
                 'FunGBE':r'D:\workissues\manuscript\halo_mining\mining\dereplication_database\FunGBE\FunGBE_V202412_for_dereplication.csv'}
    
    Deephalo_output_result = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result\20241218_OSMAC_V3\halo\test'
    ...
    for Deephalo_output in Deephalo_outputs:
        ...
        dereplication_ = Dereplication(databases, Deephalo_output_df)
