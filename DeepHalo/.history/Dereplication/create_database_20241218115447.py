...
class Dataset:
    ...
    def create_dataset(self, type):
        ...
        if 'compound_m_plus_h' not in self.df_data.columns:
            df_data_pos_3 = self.df_data[self.df_data['formula_x'].str.contains('\+3')]
            df_data_pos_3['compound_m_plus_h'] = self.df_data['mz_0']-1.007825*2
            df_data_pos_2 = self.df_data[self.df_data['formula_x'].str.contains('\+2')]
            df_data_pos_2['compound_m_plus_h'] = self.df_data['mz_0']-1.007825
            #含有+但不含+2和+3的
            df_data_pos_1 = self.df_data[~self.df_data['formula_x'].str.contains('\+2') & ~self.df_data['formula_x'].str.contains('\+3')]
            df_data_pos_1 = df_data_pos_1[df_data_pos_1['formula_x'].str.contains('\+')]
            df_data_pos_1['compound_m_plus_h'] = self.df_data['mz_0']

            df_data_neg = self.df_data[self.df_data['formula_x'].str.contains('\-')]
            df_data_neg['compound_m_plus_h'] = 0/self.df_data['mz_0']
            
            df_data_neut = self.df_data[~self.df_data['formula_x'].str.contains('\+') & ~self.df_data['formula_x'].str.contains('\-')]
            df_data_neut['compound_m_plus_h'] = self.df_data['mz_0'] + 1.007825
