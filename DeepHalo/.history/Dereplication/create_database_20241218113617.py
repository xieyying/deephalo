...
class Dataset:
    ...
    def create_dataset(self, type):
        ...
        if 'compound_m_plus_h' not in self.df_data.columns:
            formula = self.df_data.columns
            f = self.df_data['formula_x']
            print('formula:', formula)
            # if self.df_data['formula'].str.contains('+').any():
            #     self.df_data['compound_m_plus_h'] = self.df_data['mz_0']
            # elif self.df_data['formula'].str.contains('-').any():
            #     self.df_data['compound_m_plus_h'] = 0/self.df_data['mz_0']
            # else:
