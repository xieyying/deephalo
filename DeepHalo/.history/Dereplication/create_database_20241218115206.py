...
class Dataset:
    ...
    def create_dataset(self, type):
        ...
        if 'compound_m_plus_h' not in self.df_data.columns:
            ...
            df_data_pos_1 = df_data_pos_1[df_data_pos_1['formula_y'].str.contains('\+')]
            
            df_data_neg = self.df_data[self.df_data['formula_y'].str.contains('\-')]
            df_data_neut = self.df_data[~self.df_data['formula_x'].str.contains('\+') & ~self.df_data['formula_y'].str.contains('\-')]
            df_data_neut['compound_m_plus_h'] = self.df_data['mz_0'] + 1.007825
            self.df_data = pd.concat([df_data_pos, df_data_neg, df_data_neut], ignore_index=True)
