...
def add_deephalo_results_to_graphml(gnps_folder, deephalo_result):
    """
    ...
    default_Inty_cosine_score = 0
    default_error_ppm = 1e6
    default_Smiles = ''
    default_adducts = ''
    default_class_pred = -1
    ...
    halo_folder = os.path.join(deephalo_result, 'dereplication')
    ...
    for node, data in graph.nodes(data=True):
        ...
        print(f"Processing node {node} with {len(unique_file_sources)} unique file sources")
        ...
        if precursor_mass is None or rt_mean is None:
            ...
            graph.nodes[node]['Class_pred'] = int(default_adducts)
        ...
        if not halo_data_.empty:
            ...
            Inty_cosine_score = halo_data_.loc[halo_data_['Inty_cosine_score'].idxmax(), 'Inty_cosine_score']
            error_ppm = halo_data_.loc[halo_data_['Inty_cosine_score'].idxmax(), 'error_ppm']
...
if __name__ == '__main__':
    gnps_folder = r'D:\workissues\manuscript\halo_mining\mining\result\选用\53_strep_1_micomono_blank_0.6_5\merged'
    deephalo_result = r'D:\workissues\manuscript\halo_mining\mining\result\选用\53_strep_1_micromono_blank'
