...
def add_h_score_mean_to_graphml(gnps_folder, deephalo_result, default_h_score_mean=0.0):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if not halo_data_.empty:
            ...
            compound_names = halo_data_.loc[halo_data_['error_ppm'].idxmin(), 'compound_names']
            
            intensity_score = halo_data_.loc[halo_data_['error_ppm'].idxmin(), 'intensity_score']
            error_ppm = halo_data_.loc[halo_data_['error_ppm'].idxmin(), 'error_ppm']
            Smiles = halo_data_.loc[halo_data_['error_ppm'].idxmin(), 'Smiles']
        else:
        ...
        graph.nodes[node]['Smiles_'] = Smiles
        print(f"Node {node}:  compound_names = {compound_names}, intensity_score = {intensity_score}, error_ppm = {error_ppm}, Smiles = {Smiles}")
...
# Call the function
gnps_folder = r'D:\workissues\manuscript\halo_mining\mining\result\54_strep_1_micromono_GNPS'
deephalo_result = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result'
