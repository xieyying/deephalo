...
def add_h_score_mean_to_graphml(gnps_folder, deephalo_result, default_h_score_mean=0.0):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if not halo_data_.empty:
            ...
            compound_names = halo_data_['compound_names'].values[0]
            intensity_score = halo_data_['intensity_score'].values[0]
            error_ppm = halo_data_['error_ppm'].values[0]
            Smiles = halo_data_['Smiles'].values[0]
        else:
