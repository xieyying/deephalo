...
def process_file(file, args, para,  EPM, EPM_dense_output_model, ADM, blank=None,ms2=None):
    ...
    if df_f_result.shape[0] > 0:
        if file.lower().endswith(('.mzml', '.mzxml')):
            df_f_result.to_csv(os.path.join('./result/halo', os.path.basename(file).lower().replace('.mzml', '_feature.csv').replace('.mzxml', '_feature.csv')), index=False)
            df_scan_result.to_csv(os.path.join('./result/halo', os.path.basename(file).lower().replace('.mzml', '_scan.csv').replace('.mzxml', '_scan.csv')), index=False)

    if df_Se.shape[0] > 0:
        df_Se.to_csv(os.path.join('./result/Se', os.path.basename(file).lower().replace('.mzml', '_Se.csv').replace('.mzxml', '_Se.csv')), index=False)
    if df_B.shape[0] > 0:
        df_B.to_csv(os.path.join('./result/B', os.path.basename(file).lower().replace('.mzml', '_B.csv').replace('.mzxml', '_B.csv')), index=False)
    if df_Fe.shape[0] > 0:
        df_Fe.to_csv(os.path.join('./result/Fe', os.path.basename(file).lower().replace('.mzml', '_Fe.csv').replace('.mzxml', '_Fe.csv')), index=False)
    if iso_12_df is not None:
        iso_12_df.to_csv(os.path.join('./result/iso_12', os.path.basename(file).lower().replace('.mzml', '_1_or_2_iso.csv').replace('.mzxml', '_1_or_2_iso.csv')), index=False)
...
def pipeline_analyze_mzml(args,para):
    ...
    if args.blank is not None:
        ...
        if os.listdir(blank_featurexml_path) and not args.overwrite_blank:
            ...
            for b in blank_:
                file_name = os.path.basename(b.getMetaValue("spectra_data")[0].decode()).replace('.mzML','_blank.featureXML') # TODO: add mzml, mzxml and mzXML
