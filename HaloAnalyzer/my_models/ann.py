...
class pick_halo_ann(base):
    ...
    def create_Dataset(self):     
        if self.use_noise_data == 'True' or self.use_add_fe_data == 'True' or self.use_hydroisomer_data == 'True':
            train_dataset = tf.data.Dataset.from_tensor_slices((self.X_train, (self.Y_train,self.X_train_sub_group),self.X_train_weight))
        else:
            train_dataset = tf.data.Dataset.from_tensor_slices((self.X_train, (self.Y_train,self.X_train_sub_group)))
        val_dataset = tf.data.Dataset.from_tensor_slices((self.X_test, (self.Y_test,self.X_test_sub_group)))
    ...
    def train(self):
        ...
        def get_compiled_model():
            ...
            input = keras.Input(shape=(self.input_shape,), name="mass_features")
            share = layers.Dense(self.parameters['dense1'], activation="relu")(input)
            share = layers.Dropout(self.parameters['dense1_drop'])(share)
            share = layers.Dense(self.parameters['dense2'], activation="relu")(share)
            share = layers.Dropout(self.parameters['dense2_drop'])(share)

            x = layers.Dense(self.parameters['dense3'], activation="relu")(share)
            clf_base_output = layers.Dense(3,activation='softmax', name="base")(x)

            # y = layers.Concatenate()([share, clf_base_output])
            y = layers.Dense(self.parameters['dense3'], activation="relu")(share)
            clf_sub_output = layers.Dense(5,activation='softmax', name="sub")(y)

            # z2 = layers.Concatenate()([share, clf_base_output, clf_sub_output])
            # z2 = layers.Dense(self.parameters['dense3'], activation="relu")(z2)
            # clf_hydro_output = layers.Dense(2,activation='softmax', name="hydroisomer")(z2)

            clfs = keras.Model(inputs=input, outputs=[clf_base_output, clf_sub_output], name="clfs")
    ...
    def show_CM(self,pre_trained = False):#查看混淆矩阵
        ...
        wrong_index = np.where((y_val != pr_base)|(sub_val != pr_sub))
