...
def process_file(file, args, para, blank=None,ms2=None):
    ...
    df_f_result, df_scan_result,iso_12_df,df_Se,df_B,df_Fe= MyMzml(os.path.join(args.input, file), para).work_flow(blank=blank,ms2=ms2)
    path_check('./result')
    ...
    if df_f_result.shape[0] > 0:
        if file.endswith('.mzML') or file.endswith('.mzml'):
            ...
            if df_Se is not None:
                df_Se.to_csv(os.path.join('./result', os.path.basename(file).replace('.mzML', '_Se.csv').replace('.mzml', '_Se.csv')), index=False)
            if df_B is not None:
                df_B.to_csv(os.path.join('./result', os.path.basename(file).replace('.mzML', '_B.csv').replace('.mzml', '_B.csv')), index=False)
            if df_Fe is not None:
                df_Fe.to_csv(os.path.join('./result', os.path.basename(file).replace('.mzML', '_Fe.csv').replace('.mzml', '_Fe.csv')), index=False)
