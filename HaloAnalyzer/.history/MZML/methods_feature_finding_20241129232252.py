...
class FeatureDetection:
    ...
    def elution_peak_detection(self):
        ...
        self.deconvol_df = pd.DataFrame({'label': labels, 'intensity': intensities, 'size': size, 'length': length, 'convexhull': convexhull, 'smoothed_intensities': smoothed_intensities, 'mz': mz, 'RT': RT})
    ...
    def leak_feature_detection(self):
        """Detect the features"""
        masstrace_lable = [feature.getMetaValue("label").split("_") for feature in self.feature_map]
        #拉直为一维
        masstrace_lable = [item for sublist in masstrace_lable for item in sublist]
        self.deconvol_df = self.deconvol_df[~self.deconvol_df['label'].isin(masstrace_lable)]
        self.deconvol_df.to_csv(r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result\mass_traces_deconvol_dropped.csv', index=False)    
    def run(self):
