...
class FeatureDetection:
    ...
    def elution_peak_detection(self):
        ...
        print(help(self.mass_traces_deconvol[0]))
        ...
        for mass_trace in self.mass_traces_deconvol:
            ...
            mz.append(mass_trace.getMZ())
            
            
            
        df_deconvol = pd.DataFrame({'label': labels, 'intensity': intensities, 'size': size, 'length': length, 'convexhull': convexhull, 'smoothed_intensities': smoothed_intensities, 'mz': mz})
        df_deconvol.to_csv('mass_traces_deconvol.csv', index=False)
