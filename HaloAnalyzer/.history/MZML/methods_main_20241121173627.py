...
def anormal_isotope_pattern_detection(EPM, ADM, df_feature,features_list):
    """
    ...
    model_path = importlib_resources.files('HaloAnalyzer') / 'models/deephalo_ann_model.h5'
    layer_name = 'dense'
    units_1_output_model = load_trained_model(model_path, layer_name)
    autoencoder_input = units_1_output_model.predict(querys_input)
    
    autoencoder_model = importlib_resources.files('HaloAnalyzer') / 'models/anomaly_detection_model.h5'
...
def flow_base(file,model_path,pars,blank=None,ms2=None):
    ...
    df_feature_for_model_input = anormal_isotope_pattern_detection(df_feature_for_model_input,pars.features_list)
    # Filter the results based on the reconstruction error
    df_feature_for_model_input = df_feature_for_model_input[df_feature_for_model_input['reconstruction_error'] <= pars.FeatureFilter_Anomaly_detection_threshold]
    ...
    df_scan_for_model_input = anormal_isotope_pattern_detection(df_scan_for_model_input,pars.features_list)
    # Filter the results based on the reconstruction error
