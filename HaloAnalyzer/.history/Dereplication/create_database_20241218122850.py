...
from HaloAnalyzer.Dataset.methods_main import create_data,isotope_simulation
...
class Dataset:
    ...
    def create_dataset(self, type):
        ...
        """
        #isotope_simulation
        self.df_data['dict_isos'] = self.data['formula'].apply(isotope_simulation)
        self.df_data['mz_0'] = self.df_data['dict_isos'].apply(lambda x: x['mz_0'])
        self.df_data['p0_int'] = self.df_data['dict_isos'].apply(lambda x: x['p0_int'])
        self.df_data['p1_int'] = self.df_data['dict_isos'].apply(lambda x: x['p1_int'])
        self.df_data['p2_int'] = self.df_data['dict_isos'].apply(lambda x: x['p2_int'])
        self.df_data['p3_int'] = self.df_data['dict_isos'].apply(lambda x: x['p3_int'])
        self.df_data['p4_int'] = self.df_data['dict_isos'].apply(lambda x: x['p4_int'])
        #del column 'dict_isos'
        self.df_data = self.df_data.drop(columns=['dict_isos'])
