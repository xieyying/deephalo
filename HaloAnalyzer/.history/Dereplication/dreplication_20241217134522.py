        ...
        for idx,row in self.Deephalo_output.iterrows():
            ...
            if self.data_.empty:
                ...
                for idx_,row_ in self.data_.iterrows():
                    ...
                    try:
                        dereplications['error_ppm'].append(abs(row['mz']-row_['compound_m_plus_h'])/row_['mz']*1e6)
                        dereplications['error_ppm'].append(abs(row['mz']-row_['compound_m_plus_na'])/row_['mz']*1e6)
                        dereplications['error_ppm'].append(abs(row['mz']-row_['compound_m_plus_nh4'])/row_['mz']*1e6)
                    except:
                        pass
