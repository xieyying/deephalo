    ...
    def dereplication(self):
        ...
        for dataname, data in self.data.items():
            ...
            for idx, row in self.Deephalo_output.iterrows():
                ...
                data_h = data[(abs(data['M+H'] - mz) <= mz * self.error_ppm*1e-5) & (data['formula'].str.contains('Br|Cl'))]
                data_na = data[(abs(data['M+Na'] - mz) <= mz * self.error_ppm*1e-5) & (data['formula'].str.contains('Br|Cl'))]
...
if __name__ == '__main__':
    ...
    Deephalo_output_result = r'D:\workissues\manuscript\halo_mining\mining\result\选用\result_blank\test'
    ...
    for Deephalo_output in Deephalo_outputs:
        ...
        df = Dereplication(databases, Deephalo_output_df,10).workflow()
