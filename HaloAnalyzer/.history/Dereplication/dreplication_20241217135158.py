...
if __name__ == '__main__':
    ...
    for Deephalo_output in Deephalo_outputs:
        Deephalo_output = pd.read_csv(os.path.join(Deephalo_output_result,Deephalo_output))
        dereplication = dereplication(database,Deephalo_output)
        df = dereplication.dereplication()
        df.to_csv(os.path.join(Deephalo_output_result,Deephalo_output.split('_')[0]+'_dereplication.csv'),index=False)
