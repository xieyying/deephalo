...
if __name__ == '__main__':
    project_path = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result\WJH_P1_layer_3_plus_autoencoder\halo'
    ...
    # project_path = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result\halo\m1_m0_plus_m2_m1_correction\H_score_more_than_0.8'
    project_path = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result\WJH_all\halo'
    pv = PipelineVis(project_path)
    pv.main()
    
    # files = os.listdir(project_path)
    # files = [file for file in files if file.endswith('feature.csv')]
    # output = project_path + '\\inty_more_than_e6'
    # for f in files:
    #     df = pd.read_csv(project_path + '\\' + f)
    #     # df = df[(df['m1_m0']<1.0123) & (df['m1_m0']>0.9939) & (df['m2_m1']<1.0104) & (df['m2_m1']>0.9874) & (df['reconstruction_error']<0.0004)]
    #     # df = df[df['H_score']>0.8]
    #     df = df[df['intensity']>1e6]
    #     if df.shape[0] > 0:
