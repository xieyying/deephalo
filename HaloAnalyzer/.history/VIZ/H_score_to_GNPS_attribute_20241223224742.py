...
def add_deephalo_results_to_graphml(gnps_folder, deephalo_result):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if not halo_data_.empty:
            ...
            print(halo_data_.loc[halo_data_['Inty_cosine_score'].idxmax(), 'Inty_cosine_score'])
            Inty_cosine_score = round(float(halo_data_.loc[halo_data_['Inty_cosine_score'].idxmax(), 'Inty_cosine_score']),2)
            error_ppm = round(float(halo_data_.loc[halo_data_['Inty_cosine_score'].idxmax(), 'error_ppm']),1)
