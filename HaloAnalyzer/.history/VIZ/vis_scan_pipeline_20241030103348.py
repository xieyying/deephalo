...
class PipelineVis:
    ...
    def load_base_data(self, file_path):
        ...
        self.scan['mz_list'] = self.scan['mz_list'].apply(lambda x: list(ast.literal_eval(x)))
        self.scan['inty_list'] = self.scan['inty_list'].apply(lambda x: list(ast.literal_eval(x)))
        self.scan['RT'] = self.scan['RT']/60
        # Step 2: Convert 'inty_list' column values from tuples to lists
        # 展开mz_list和inty_list
        rows = []
        for _, row in self.scan.iterrows():
            for mz, inty in zip(row["mz_list"], row["inty_list"]):
    ...
    def main(self):
        ...
        files = [file for file in files if file.endswith('scan.csv')]
...
if __name__ == '__main__':
    project_path = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\2M_fake_molecules\result'
