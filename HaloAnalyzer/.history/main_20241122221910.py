...
def process_file(file, args, para,  EPM, EPM_dense_output_model, ADM, blank=None,ms2=None):
    ...
    if iso_12_df is not None:
        iso_12_df.to_csv(os.path.join('./result/Fe/1_or_2_iso', os.path.basename(file).replace('.mzML', '_1_or_2_iso.csv').replace('.mzml', '_1_or_2_iso.csv')), index=False)
...
def pipeline_analyze_mzml(args,para):
    ...
    path_check('./result/Fe/1_or_2_iso')
