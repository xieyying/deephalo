...
def halo_scoring(df_f_, df_scan_):
    ...
    for i in df_scan['feature_id_flatten'].unique():
        ...
        print(type(scan_predictions), type(feature_prediction))
        print(f"Processing feature {i}: scan_predictions={scan_predictions}, feature_prediction={feature_prediction}")
        h_score, sub_class, r_halo, z_score, f_score = HalogenScorer().process_feature(scan_predictions, feature_prediction)
        # print(f"Processing feature {i}: h_score={h_score}, sub_class={sub_class}, r_halo={r_halo}, z_score={z_score}, f_score={f_score}")
...
if __name__ == "__main__":
    ...
    # df_f_ = pd.read_csv(os.path.join(work_folder, 'Strepomyces_cmx_11_23_M9_cmx_p9_C9_nr1_feature.csv'))
    # df_scan_ = pd.read_csv(os.path.join(work_folder, 'Strepomyces_cmx_11_23_M9_cmx_p9_C9_nr1_scan.csv'))
    # df_f_['classification'] = df_f_['Feature_based_prediction']
    # df_scan_['classification'] = df_scan_['Feature_based_prediction']
    
    # df_f, df_scan = halo_scoring(df_f_, df_scan_)
    # df_f.to_csv(os.path.join(work_folder, 'Strepomyces_cmx_11_23_M9_cmx_p9_C9_nr1_feature_halo.csv'), index=False)
    # df_scan.to_csv(os.path.join(work_folder, 'Strepomyces_cmx_11_23_M9_cmx_p9_C9_nr1_scan_halo.csv'), index=False)
    
    
    scan_predictions = [7, 6, 6, 6, 6, 6, 7, 6, 7, 6]
    feature_prediction = 6
    halogen_scorer = HalogenScorer()
    h_score, sub_class, r_halo, z_score, f_score = halogen_scorer.calculate_overall_score(scan_predictions, feature_prediction)
    print(f"Halogen Score: {h_score}")
