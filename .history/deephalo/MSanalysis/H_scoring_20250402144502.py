...
class HalogenScorer:
    ...
    def evaluate_scan_prediction(self, scan_predictions):
        """
        ...
        if any(pred in self.halogen_classes for pred in scan_predictions):
            classification = 'halo'
            score = self.calculate_prediction_consistency(scan_predictions)
            if halo_ratio(scan_predictions) > 0.5:
                _scan_predictions = [pred for pred in scan_predictions if pred in self.halogen_classes]
                sub_classification = max(Counter(_scan_predictions).items(), key=lambda x: x[1])[0]
            else:
