...
def process_file(file, args, para, blank=None,ms2=None):
    ...
    df_f_result, df_scan_result,df_1_or_2_iso = MyMzml(os.path.join(args.input, file), para).work_flow(blank=blank,ms2=ms2)
    ...
    if df_f_result.shape[0] > 0:
        if file.endswith('.mzML') or file.endswith('.mzml'):
            ...
            df_1_or_2_iso.to_csv(os.path.join('./result', os.path.basename(file).replace('.mzML', '_1_or_2_iso.csv').replace('.mzml', '_1_or_2_iso.csv')), index=False)
