...
class NoveltyDetection:
    def __init__(self, features, train_path, test_path) -> None:
        ...
        max =np.log(3000)
        train['normalized_mz'] = np.log(train['mz_0'])/max
        test = pd.read_csv(test_path)
        test['normalized_mz'] = np.log(test['mz']*test['charge'])/max
        self.features = features
        # self.train = train[train['group'].isin([0, 1, 2, 6])]
        self.train = train[train['group'].isin([0, 1, 2,])]
        self.test = test
    ...
    def model_predict_and_evaluate(self):
        ...
        self.test[self.predictions == -1].to_csv(f'C:\\Users\\xyy\\Desktop\\tem\\{self.method_name}_test_0.000001_0126_hmw.csv',index=False)
...
if __name__ == "__main__":
    ...
    test_file = r'C:\Users\xyy\Desktop\python\HaloAnalyzer_training\022_six_dataset_openms_noClFe\result\2ppm_2e4_high_mw_feature.csv'
    test_file = r'D:\workissues\manuscript\halo_mining\HaloAnalyzer\Simulated_LC_MS\LC_MSMS_data_from_papers\simulated_mzml\test\2ppm_2e4_feature.csv'
