...
class FeatureMapProcessor(FeatureDetection):
    ...
    def process_feature(self, feature):
        ...
        if feature.getMetaValue("num_of_masstraces") >= self.pars.FeatureMapProcessor_min_num_of_masstraces and \
           ...
           len(feature.getConvexHulls()[0].getHullPoints()) > self.pars.FeatureMapProcessor_min_scan_number:
            return self._process_feature(feature)
        return None
    ...
    def process(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = list(executor.map(self.process_feature, filtered_features))
        ...
        for result in results:
            if result:
                ...
                self.masstrace_intensity.append(masstrace_intensity)
                self.masstrace_centroid_mz.append(masstrace_centroid_mz)
                self.masstrace_lable.append(masstrace_lable)
