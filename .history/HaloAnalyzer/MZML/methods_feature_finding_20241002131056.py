...
class FeatureMapProcessor(FeatureDetection):
    ...
    def process(self):
        ...
        for feature in self.feature_map:
            num_of_masstraces = feature.getMetaValue("num_of_masstraces")
            intensity = feature.getIntensity()
            scan_number = len(feature.getConvexHulls()[0].getHullPoints())
            
            if num_of_masstraces >= min_num_of_masstraces and \
            intensity >= min_feature_int and \
            scan_number > min_scan_number:
                self._process_feature(feature)
