...
class FeatureMapProcessor(FeatureDetection):
    ...
    def _grouping_df_based_on_feature_plus_scan(self):
        ...
        # Extract mz_list and inty_list
        mz_list = [list(zip(*item))[0] for item in sorted_mz_inty]
        inty_list = [list(zip(*item))[1] for item in sorted_mz_inty]
        ...
        sorted_mz_inty = [sorted(zip(mz, inty)) for mz, inty in zip(self.df_scan['mz_list'], self.df_scan['inty_list'])]
        self.df_scan['mz_list'] = [list(zip(*item))[0] for item in sorted_mz_inty]
        self.df_scan['inty_list'] = [list(zip(*item))[1] for item in sorted_mz_inty]
