...
class FeatureDetection:
    ...
    def mass_trace_detection(self):
        ...
        #     mz.append(self.mass_traces[i].getCentroidMZ())
        #     rt.append(self.mass_traces[i].getCentroidRT())
        #     intensity.append(self.mass_traces[i].getIntensity(1))
        # self.df = pd.DataFrame({'mz':mz,'RT':rt,'intensity':intensity})
        # self.df.to_csv(r'./mass_traces.csv')  
        
    def elution_peak_detection(self):
        """Detect the elution peaks"""
        epd = oms.ElutionPeakDetection()
        epd_par = epd.getDefaults()
        # print('elution parameter',epd_par)
        ...
        # self.df = pd.DataFrame({'mz':mz,'RT':rt,'intensity':intensity})
        # self.df.to_csv(r'./mass_traces_deconvol.csv')    
    ...
    def feature_detection(self):
        ...
        #save feature map
        # oms.FeatureXMLFile().store(r'./feature_map.featureXML', self.feature_map)
...
class FeatureMapProcessor(FeatureDetection):
    ...
    def _transform_to_dataframe(self):
        ...
        self.df_feature["masstrace_centroid_mz"] = self.masstrace_centroid_m
    ...
    @njit
    def sort_and_zip(mz, inty):
        sorted_indices = np.argsort(mz)
        return mz[sorted_indices], inty[sorted_indices]

    @njit(parallel=True)
    def process_groups(mz_lists, inty_lists, charges):
        mz_result = []
        inty_result = []
        mz_final = np.empty(len(mz_lists))
        
        for i in prange(len(mz_lists)):
            mz_sorted, inty_sorted = sort_and_zip(mz_lists[i], inty_lists[i])
            mz_result.append(mz_sorted)
            inty_result.append(inty_sorted)
            mz_final[i] = mz_sorted[0] * charges[i] - (charges[i] - 1) * 1.007276466812
        
        return mz_result, inty_result, mz_final

    def _grouping_df_based_on_feature_plus_scan(self):
        """Group the dataframe based on FeaturePlusScan"""
        grouped = self.df_feature_flatten.groupby(['feature_id_flatten', 'RT'])
        
        # Precompute sorted mz and inty lists
        sorted_mz_inty = grouped.apply(lambda x: x.sort_values('mz_type')[['mz', 'inty']].values.tolist())
        sorted_mz_inty = sorted_mz_inty.reset_index(drop=True)
        
        # Extract mz_list and inty_list using numpy for better performance
        mz_list = [np.array(item)[:, 0] for item in sorted_mz_inty]
        inty_list = [np.array(item)[:, 1] for item in sorted_mz_inty]
        
        # Extract charge
        charge = grouped['charge_f'].first().reset_index(drop=True).values
        
        # Process groups using Numba
        mz_list, inty_list, mz_final = process_groups(np.array(mz_list), np.array(inty_list), charge)
        
        # Create the dataframe
        self.df_scan = pd.DataFrame({
            'feature_id_flatten': grouped['feature_id_flatten'].first().reset_index(drop=True),
            'RT': grouped['RT'].first().reset_index(drop=True),
            'mz_list': mz_list,
            'inty_list': inty_list,
            'charge': charge,
            'mz': mz_final
    ...
    def process(self):
        ...
        print('merge feature is done')
        self._add_intensity()
        print('add intensity is done')
        self._grouping_df_based_on_feature_plus_scan()
        print('grouping df is done')
