...
class FeatureMapProcessor(FeatureDetection):
    ...
    def _grouping_df_based_on_feature_plus_scan(self):
        ...
        # Extract charge
        charge = grouped['charge_f'].first().reset_index(drop=True)
        ...
        # Sort mz_list and inty_list together using numpy
        sorted_mz_inty = [np.array(sorted(zip(mz, inty))) for mz, inty in zip(self.df_scan['mz_list'], self.df_scan['inty_list'])]
        self.df_scan['mz_list'] = [item[:, 0] for item in sorted_mz_inty]
        self.df_scan['inty_list'] = [item[:, 1] for item in sorted_mz_inty]
        
        # Compute mz using numpy
        mz = np.array([item[0] for item in self.df_scan['mz_list']])
        self.df_scan['mz'] = mz * self.df_scan['charge'].values - (self.df_scan['charge'].values - 1) * 1.007276466812
