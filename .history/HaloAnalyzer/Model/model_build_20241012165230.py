...
def model(input_shape,  output_shape):
    ...
    input1 = input[:,:-2]
    input1 = layers.GaussianNoise(0.04)(input1)
   
    input2 = input[:,-2:]
    input2 = layers.GaussianNoise(0.005)(input2)
    input2 = tf.pow(input2, 20)
    
    x = layers.Dense(16, activation="relu")(input1) # units_0x
    y = layers.Dense(512, activation="relu")(input2)  # units_0y

    share = layers.concatenate([x, y])  
    share = layers.Dense(32, activation="relu")(share) # units_1
    # share = layers.Dropout(.3)(share)  # dropout_1
    share = layers.Dense(64, activation="relu")(share) # units_2
    ...
    share = layers.Dense(32, activation="relu")(share) # units_4
