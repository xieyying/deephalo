...
def add_deephalo_results_to_graphml(gnps_folder, deephalo_result_dereplication_folder):
    ...
    for node, data in graph.nodes(data=True):
        ...
        for f in halo_files_names:
            ...
            halo_data = halo_data[(halo_data['mz'] > (precursor_mass - 0.05)) & (halo_data['mz'] < (precursor_mass + 0.05))]
            if len(halo_data) > 1:
                # Extract data from halo_data where mz is close to precursor_mass
        ...
        if not halo_data_.empty:
            ...
            if halo_data_['compound_names'].notnull().any():
                #获取不为None的元素的index
                print(f"Processing node {node},compound_names:{halo_data_['compound_names']}")
                ...
                Inty_cosine_score = str(halo_data_.loc[idx, 'Inty_cosine_score'].tolist())
