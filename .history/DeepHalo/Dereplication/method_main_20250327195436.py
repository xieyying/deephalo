    ...
    for node, data in graph.nodes(data=True):
        ...
        if not halo_data_.empty:
            ...
            if compound_names_.any():
                ...
                if len(compound_names_.unique()) == 1 and len(idx)/len(halo_data_) > 0.5:
                    ...
                    Inty_cosine_score = str(np.round(mean_score, 4).tolist()[0])
                    compound_names = str(halo_data_['compound_names'][idx].unique()[0])
                    error_series = halo_data_['error_ppm'].loc[idx].apply(parse_inty)
                    error_arr = np.array(error_series.tolist())
                    mean_error = np.mean(error_arr, axis=0)
                    error_ppm = str(np.round(mean_error, 4).tolist()[0])
                    Smiles = str(halo_data_['Smiles'][idx].unique()[0])
                    Adducts = str(halo_data_['adducts'][idx].unique()[0])
                    print(f"node:{node},compound_names:{compound_names},Inty_cosine_score:{Inty_cosine_score},error_ppm:{error_ppm},Smiles:{Smiles},Adducts:{Adducts}")
