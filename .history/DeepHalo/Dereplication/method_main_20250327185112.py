        ...
        if not halo_data_.empty:
            ...
            if halo_data_['compound_names'].notnull().any():
                ...
                if len(idx)/len(halo_data_) > 0.5:
                    ...
                    #如果是数字，取平均值
                    if halo_data_['Inty_cosine_score'][idx].dtype == 'float64':
                        Inty_cosine_score = round(float(halo_data_['Inty_cosine_score'][idx].mean()), 4)
                        print(f"Processing node {node},Inty_cosine_score:{Inty_cosine_score}")
                        
                    else:
                        print(type(halo_data_['Inty_cosine_score'][idx]))
                        print(halo_data_['Inty_cosine_score'][idx])
