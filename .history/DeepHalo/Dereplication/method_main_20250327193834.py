...
def add_deephalo_results_to_graphml(gnps_folder, deephalo_result_dereplication_folder):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if not halo_data_.empty:
            ...
            compound_names_ = halo_data_['compound_names'].notnull()
            if compound_names_.any():
                idx = halo_data_[compound_names_].index
                if len(compound_names_.unique()) > 1:
                    compound_names = str(halo_data_['compound_names'][idx].tolist())
                    Inty_cosine_score = str(halo_data_['Inty_cosine_score'][idx].tolist())
                    error_ppm = str(halo_data_['error_ppm'][idx].tolist())
                    Smiles = str(halo_data_['Smiles'][idx].tolist())
                    Adducts = str(halo_data_['adducts'][idx].tolist())
                
                if len(idx)/len(halo_data_) > 0.5:

                    # Parse the values at the given index 'idx'
                    scores_series = halo_data_['Inty_cosine_score'].loc[idx].apply(parse_inty)
                    scores_arr = np.array(scores_series.tolist())
                    mean_score = np.mean(scores_arr, axis=0)
                    Inty_cosine_score = round(mean_score, 4)

                    try:
                        Inty_cosine_score = halo_data_['Inty_cosine_score'][idx].apply(lambda x: ast.literal_eval(x) if type(x) is str else float(x))
                        Inty_cosine_score =round(np.mean(np.array(Inty_cosine_score.tolist()) , axis=0),4)
                        print(f"Processing node {node},Inty_cosine_score:{Inty_cosine_score}")
                        compound_names = str(halo_data_['compound_names'][idx])
                        
                        
                    except:
                        print(f"Processing node {node},Inty_cosine_score:{halo_data_['Inty_cosine_score']}")
